...
public class GameManager implements Serializable
{
    ...
    public GameManager(Game gameActivity,int subNum)
    {
        ...
        this.board=new int[UpClass.Size][UpClass.Size];
...
//    }
    ...
    public ArrayList<Position> possiblePlaces(int subSize) //פעולה שמחזירה arraylist עם כל המיקומים האפשריים לצוללת והכיוונים בהתחשב בגודל הצוללת
    {
        ...
        for (int i = 0; i< UpClass.Size-subSize; i++)
        {
            for(int j = 0; j< UpClass.Size; j++)
            {
                if (UpClass.canPlaceVertical(this.board,i,j,subSize))
                    arrlst.add(new Position(i,j,false));
            }
        }
        for (int i = 0; i< UpClass.Size; i++)
        {
            for(int j = 0; j< UpClass.Size-subSize; j++)
            {
                if (UpClass.canPlaceHorizontal(this.board,i,j,subSize))
